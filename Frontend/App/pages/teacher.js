...
export default function DashboardWrapper({ navigation, route }) {
  ...
  const [aboutMe, setAboutMe] = useState("You haven't added about you.");
  const [subjects, setSubjects] = useState("You haven't added any subjects.");
  const [favoriteTopics, setFavoriteTopics] = useState("You haven't added any favorite topic.");
  const [socialMedia, setSocialMedia] = useState("You haven't added any social media profiles.");
  const [activities, setActivities] = useState([
    { 
      time: '5m ago', 
      action: 'Jared Otieno started following you' 
    },
    { 
      time: 'Today 7:51 pm', 
      action: 'Jeff Omondi posted something on your timeline' 
    },
    { 
      time: '30m ago', 
      action: 'Jeff Omondi: Mathematics isn\'t just numbers and equations; it\'s a language of logic and problem-solving...' 
    },
    { 
      time: 'Today 5:35 pm', 
      action: 'Christina Mason' 
    },
    { 
      time: '2 days ago', 
      action: 'Christina Mason' 
        ...
        <View style={styles.mainContent}>
            ...
            {editing ? (
              <View style={styles.profileDetails}>
                <Text style={styles.profileText}>Omondi Jeff</Text>
                <Text style={styles.profileText}>Teacher/Tutor</Text>
                ...
                />
                <Text style={styles.sectionTitle}>Favorites Topics</Text>
                <TextInput
                  style={styles.input}
                  value={favoriteTopics}
                  onChangeText={text => setFavoriteTopics(text)}
                />
                ...
                {activities.map((activity, index) => (
                  <Text key={index} style={styles.activity}>{activity.time} {activity.action}</Text>
                ))}
